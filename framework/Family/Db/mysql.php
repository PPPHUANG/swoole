...
class Mysql
{
    ...
    public function connect($config)
    {
        ...
        if (!empty($config['slave'])) {
            //创建从数据库连接
            foreach ($config['slave'] as $config) {
                $slave = new MySql();
                $res = $slave->connect($config);
                if ($res === false) {
                    //连接失败，抛异常
                    throw new \Exception($slave->connect_error, $slave->connect_errno);
                } else {
                    //存入slave资源
                    $this->slave[] = $slave;
                }
    ...
    public function reconnect($type, $index)
    {
        ...
        if (!empty($this->config['slave'])) {
            //创建从数据库连接
            $slave = new SwMySql();
            $res = $slave->connect($config['slave'][$index]);
            if ($res === false) {
                //连接失败，抛异常
     ...
     * @param $arguments
     * @return mixed
     * @desc 利用__call，实现操作MySQL，并能做短线重连等相关操作
     * @throws \Exception
     */
     ...
     * @param $sql
     * @desc 根据sql语句，选择主还是从
     * @desc 判断是否是select 则选择从库，insert，update，delete等选择主库
     * @return array
     */
